...
function prepareGameField() {
  ...
  for (let i = 0; i < sizeSelector.value; i++) {
    ...
    card.innerHTML = `<div class="card__front" style="background-image: url(img/${
      cardThemeSelected.value
    }_bg.jpg);"></div><div class="card__back" style="background-image: url(img/${
      cardThemeSelected.value
    }_${numbersArray[i] + 1}.jpg);"></div>`;
...
function checkCard({ target }) {
  ...
  setTimeout(function () {
    if (!cardSelected) {
      ...
      numbersArray[cardSelected - 1] ==
        numbersArray[cardsArray.indexOf(target)] &&
      cardSelected - 1 != cardsArray.indexOf(target)
    ) {
